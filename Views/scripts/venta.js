...
function init(){
   ...
   $.post("../Controllers/venta.php?op=selectCliente", function(r){
   	$("#idcliente").html(r);
   	$('#idcliente').selectpicker('refresh'); 
   });

   //cargamos los items al celect comprobantes
   $.post("../Controllers/venta.php?op=selectComprobante", function(c){ 
...
function serie_comp(){
	...
	$.post("../Controllers/venta.php?op=mostrar_serie",{tipo_comprobante : tipo_comprobante},
		function(data,status)
		{
			data=JSON.parse(data);
			//alert(data.letra);
			$("#serie_comprobante").val( ('000' + data.serie).slice(-3) ); // "0001"
...
function numero_comp(){
	var tipo_comprobante = $("#tipo_comprobante").val();
	$.ajax({
	url: '../Controllers/venta.php?op=mostrar_numero',
...
$.ajax({
url: '../Controllers/negocio.php?op=mostrar_impuesto',
type:'get',
dataType:'json',
success: function(i){
	 impuesto=i;
...
//declaramos variables necesarias para trabajar con las V y sus detalles
var cont=0;
var detalles=0;
$("#btnGuardar").hide();
...
function listar(){
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":
		{
			url:'../Controllers/venta.php?op=listar',
			type: "get",
			dataType : "json",
			error:function(e){
...
function listarMuebles(){
	tabla=$('#tblmuebles').dataTable({
		...
		"ajax":
		{
			url:'../Controllers/venta.php?op=listarMuebles',
			type: "get",
			dataType : "json",
			error:function(e){
				console.log(e.responseText);
...
function guardaryeditar(e){
     ...
     $.ajax({
     	url: "../Controllers/venta.php?op=guardaryeditar",
     	type: "POST",
     	data: formData,
     	contentType: false,
     	processData: false,
...
function mostrar(idventa){
	...
	$.post("../Controllers/venta.php?op=mostrar",{idventa : idventa},
		function(data,status)
		{
			data=JSON.parse(data);
			//mostrarform(true);
	...
	$.post("../Controllers/venta.php?op=listarDetalle&id="+idventa,function(r){
		$("#detallesm").html(r);
	});
...
function anular(idventa){
	bootbox.confirm("¿Esta seguro de desactivar este dato?", function(result){
		if (result) {
			$.post("../Controllers/venta.php?op=anular", {idventa : idventa}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function agregarDetalle(idmueble,mueble,precio_venta,cantidad){
	var stock=cantidad;
	var descuento=0;
	var letras_s='Stock:'
...
$("#numeros").html('000');
    ...
    $(".click").click(function(e) {
        ...
        '<td><input type="hidden" name="idarticulo[]" value="'+idmueble+'">'+mueble+
        '<td><span id="mensaje" name="'+stock+'" class="click bg-danger">'+letras_s+' '+stock+' <i class="fa fa-arrow-circle-left"></i> '+'<input type="number" onchange="subir(this.value)" class="mostrar" name="cantidad[]" id="cantidad[]" value="'+numero_cantidad+'"></span></td>'+
        '<td><input type="number" step="0.01" onchange="modificarSubtotales()" name="precio_venta[]" id="precio_venta[]" value="'+precio_venta+'"></td>'+
        '<td><input type="number" step="0.01" onchange="modificarSubtotales()" name="descuento[]" value="'+descuento+'"></td>'+
...
function subir(valor){
	...
	if (valor>numero_cantidad) {
		bootbox.alert(valor+' '+msj+' '+numero_cantidad);
		 $("#btnGuardar").hide();
	}else{
	$("#btnGuardar").show();
	 modificarSubtotales();	
