...
switch ($_GET["op"]) {
	...
	case 'anular':
		...
		echo $rspta ? "Compra anulado correctamente" : "No se pudo anular el Compra";
		break;
    ...
    case 'listar':
		...
		case 'selectCliente':
			...
			$mueble=new Mueble();
				...
				$rspta=$mueble->listarActivosVenta();
		$data=Array();

		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idmueble.',\''.$reg->nombre.'\','.$reg->precio_venta.','.$reg->stock.')"><span class="fa fa-plus"></span></button>',
            "1"=>$reg->nombre,
            "2"=>$reg->categoria,
            "3"=>$reg->codigo,
            "4"=>$reg->stock,
            "5"=>$reg->precio_venta,
            "6"=>"<img src='../files/muebles/".$reg->imagen."' height='50px' width='50px'>"
