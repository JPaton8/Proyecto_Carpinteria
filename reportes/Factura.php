...
class PDF_Invoice extends FPDF
{
    ...
    function addSociete($nom, $adresse, $logo, $ext_logo)
    {
        ...
        $this->Image($logo, 10, 10, 25, 25, $ext_logo);
        $this->SetXY($x1, $y1);
    ...
    function fact_dev($libelle, $num, $nombre, $cod)
{
    $r1  = $this->w - 80;
    $r2  = $r1 + 68; // Reduje el ancho del rectángulo aún más
    $y1  = 7;
    $y2  = $y1 + 2;
    $mid = ($r1 + $r2) / 2;

    // Combina el número de factura y el código de factura en una cadena con salto de línea
    $texte = $libelle . utf8_decode(" N° : ") . $num . "\n" . "Codigo Actorizacion" . utf8_decode("  : ") . $cod;

    // Configuración para el texto combinado
    $szfont = 10; // Aumenté el tamaño de la fuente
    $loop   = 0.2;
    ...
    // Dibujar rectángulo y texto para el número y nombre
    $this->SetLineWidth(0.3);
    $this->SetFillColor(220, 220, 220);
    $this->RoundedRect($r1, $y1, ($r2 - $r1), $y2 * 2, 4, 'DF'); // Reduje el ancho del rectángulo
    ...
    function addDate($date)
{
    ...
    $r2  = $r1 + 68;
    $y1  = 28;
    $y2  = $y1 - 6; // Ajusté la altura del rectángulo dividido
    $mid = $y1 + ($y2 / 2);
    ...
    // Configurar posición y estilo para el texto "FECHA"
    ...
    // Configurar posición y estilo para la fecha
    $this->SetXY($r1 + ($r2 - $r1) / 2 - 7, $y1 + 13.5);
    $this->SetFont("Arial", "", 11.5);

    // Escribir la fecha en el rectángulo inferior
    $this->Cell(12, 7, $formattedDate, 0, 0, "C");
}
...
*/
    ...
    function addLine($ligne, $tab)
    {
        ...
        foreach ($colonnes as $lib => $pos) {
            ...
            $this->MultiCell($longCell, 4, $texte, 0, 'C', false);
    ...
    function addCadreTVAs($letras, $cantidad)
    {
        ...
        /*$arreglo = str_split($letras, $cantidad);
