...
require_once "../Models/Codigo.php";
$ncodigo= new Codigo();
$rsptan = $ncodigo->listar();
$regc=$rsptan->fetch_object();
$codigo_auth = $regc->codigo;
...
$pdf->addSociete(iconv('UTF-8', 'ISO-8859-1', $empresa),
                 $ndocumento. ": ".$documento."\n".
                 utf8_decode("Direccion: "). utf8_decode($direccion)."\n".
                 utf8_decode("Telefono: ").$telefono."\n".
                 "Email: ".$email,$logoe,$ext_logo);

$pdf->fact_dev("$regv->tipo_comprobante ","$regv->num_comprobante","$regc->nombre","$regc->codigo");
$pdf->temporaire( "      " );
...
while($regd=$rsptad->fetch_object()){
  $line = array( "CODIGO"=>"$regd->codigo",
                 "DESCRIPCION"=>utf8_decode("$regd->mueble"),
...
$con_letra=strtoupper(convertirNumeroLetra($total,$moneda));
